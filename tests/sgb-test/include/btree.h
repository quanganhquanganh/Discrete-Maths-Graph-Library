...
extern int btcls(BTA *);// free cay
extern BTA *btcrt(char *,int, int); // tao ra mot tep Btree moi ( 2 cai int kia deu phai bo bang 0)
extern int btchgr(BTA *,char *);
extern int btdups(BTA *,BTint);
extern int btinit(void);
/* Hàm btinit khởi tạo thư viện B Tree. Nó phải được gọi trước bất kỳ quy trình B Tree nào khác. Nếu không làm như vậy sẽ dẫn đến các lỗi lạ.
 */
extern BTA *btopn(char *,int,int);// mo tep Btree hien co ( 2 tham so int deu phai bang 0 )
